...
def compute_map_at_iou(pred_by_video, gt_dict, iou_threshold=0.5):
    """
    Compute mean Average Precision (mAP) at a given IoU threshold.
    pred_by_video: dict {video_id: [pred_tracks]}
    gt_dict: dict {video_id: [gt_tracks]}
    ...
    for video_id in set(list(gt_dict.keys()) + list(pred_by_video.keys())):
        ...
        gt_matched = [False] * len(gt_tracks)
        pred_scores = [p.get('score', 1.0) for p in pred_tracks]
        # Compute IoU matrix
        ...
        for j, pred_ann in enumerate(pred_tracks):
            ...
            if best_gt >= 0:
                ...
                matches.append(0)  # FP
        # Add FNs for unmatched GTs
        num_fn = gt_matched.count(False)
        all_scores.extend(pred_scores)
        all_matches.extend(matches)
        all_matches.extend([None] * num_fn)  # None for FN (not used in precision, but for recall)
    ...
    for idx in sorted_indices:
        if all_matches[idx] == 1:
            tp += 1
        else:
            fp += 1
        precisions.append(tp / (tp + fp))
        recalls.append(tp / num_gt_total if num_gt_total > 0 else 0)
    ap = 0.0
    for t in np.linspace(0, 1, 101):
        p = max([prec for prec, rec in zip(precisions, recalls) if rec >= t] or [0])
        ap += p / 101
    return ap
...
def main(results_json, val_json, csv_path="mask_metrics.csv"):
    ...
    for pred in preds:
        if pred.get('category_id') == 13 and pred.get('score', 0) >= 0.7:
            vid = pred['video_id']
    ...
    for video_id in tqdm(video_ids, desc="Processing videos"):
        ...
        for gt_idx, pred_idx in zip(gt_indices, pred_indices):
            ...
            mean_iou = np.mean(frame_ious) if frame_ious else 0.0
            mean_f = np.mean(frame_fmeasures) if frame_fmeasures else 0.0
            video_frame_ious.append(mean_iou)
            video_frame_fmeasures.append(mean_f)
        video_ious[video_id] = np.mean(video_frame_ious) if video_frame_ious else 0.0
        video_fmeasures[video_id] = np.mean(video_frame_fmeasures) if video_frame_fmeasures else 0.0
    ...
    map50 = compute_map_at_iou(pred_by_video, gt_dict, iou_threshold=0.5)
    map75 = compute_map_at_iou(pred_by_video, gt_dict, iou_threshold=0.75)
...
if __name__ == "__main__":
    main(results_json='/home/simone/fish-dvis/dvis-model-outputs/trained_models/dvis_daq_pretrained/inference/results.json', 
         val_json='/home/simone/shared-data/fishway_ytvis/val copy.json', 
         csv_path='/home/simone/fish-dvis/dvis-model-outputs/trained_models/dvis_daq_pretrained/inference/mask_metrics.csv')
