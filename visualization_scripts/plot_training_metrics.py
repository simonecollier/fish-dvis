...
model_dir = "/home/simone/store/simone/dvis-model-outputs/trained_models/enhanced_augmentations_0.0001_15f"
...
# Create cleaner individual plots for each loss type
fig, axes = plt.subplots(1, 3, figsize=(18, 6))
fig.suptitle('Training Losses by Fish Species - Clean View', fontsize=16, fontweight='bold')

# Cross-Entropy Loss
ax1 = axes[0]
for i in range(5):
    if len(loss_ce_by_class[i]) > 0:
        ax1.plot(iterations[:len(loss_ce_by_class[i])], loss_ce_by_class[i], 
                color=colors[i], linewidth=2, label=fish_species[i])
ax1.set_xlabel('Iteration')
ax1.set_ylabel('Cross-Entropy Loss')
ax1.set_title('Cross-Entropy Loss by Species')
ax1.legend()
ax1.grid(True, alpha=0.3)
...
for i in range(5):
    if len(loss_dice_by_class[i]) > 0:
        ax2.plot(iterations[:len(loss_dice_by_class[i])], loss_dice_by_class[i], 
                color=colors[i], linewidth=2, label=fish_species[i])
ax2.set_xlabel('Iteration')
ax2.set_ylabel('Dice Loss')
ax2.set_title('Dice Loss by Species')
ax2.legend()
ax2.grid(True, alpha=0.3)
...
for i in range(5):
    if len(loss_mask_by_class[i]) > 0:
        ax3.plot(iterations[:len(loss_mask_by_class[i])], loss_mask_by_class[i], 
                color=colors[i], linewidth=2, label=fish_species[i])
ax3.set_xlabel('Iteration')
ax3.set_ylabel('Mask Loss')
ax3.set_title('Mask Loss by Species')
ax3.legend()
ax3.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(model_dir, "clean_training_losses_by_species.png"), 
            dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(model_dir, "main_training_loss_by_species.png"), 
            dpi=300, bbox_inches='tight')
plt.show()

print(f"Plots saved with {len(iterations)} iterations of training data")
